...
class BatchLoader:
    ...
    def build_word_vocab(self, sentences):
        ...
        # Build vocabulary
        word_counts = collections.Counter(sentences) #统计词频

        # Mapping from index to word
        idx_to_word = [x[0] for x in word_counts.most_common()]#31252 按词频排序编号，词频越小的id越小
        idx_to_word = list(sorted(idx_to_word)) + [self.pad_token, self.go_token, self.end_token] #最后加上三个token
    ...
    def preprocess(self, data_files, idx_files, tensor_files):
        ...
        data_words = [[line.split() for line in target.split('\n')] for target in data] #分出来（每个句子，每个词）
        merged_data_words = merged_data.split() #所有的词

        self.words_vocab_size, self.idx_to_word, self.word_to_idx = self.build_word_vocab(merged_data_words)
        self.max_word_len = np.amax([len(word) for word in self.idx_to_word])#最大单词长度47
        self.max_seq_len = np.amax([len(line) for target in data_words for line in target]) #最大句子长度55个词
        self.num_lines = [len(target) for target in data_words]#总句子数

        with open(idx_files[0], 'wb') as f:
            cPickle.dump(self.idx_to_word, f)

        self.word_tensor = np.array(# 《数据的每个句子，每个句子中每个词的索引）
        ...
        self.just_words = [word for line in self.word_tensor[0] for word in line]# 这里按照整个数据上词的顺序存成了一个列表
...
#         print encoder_word_input
    ...
    def next_embedding_seq(self, seq_len):#seq_len是embedding窗口的大小
        """
        :return:
        ...
        words_len = len(self.just_words)#总单词数
        seq = [self.just_words[i % words_len]
               for i in np.arange(self.word_embedding_index, self.word_embedding_index + seq_len)]

        result = []
        for i in range(seq_len - 2):
            result.append([seq[i + 1], seq[i]])#当前位置前面的词是什么
            result.append([seq[i + 1], seq[i + 2]])#当前位置后面的词是什么
        ...
        #print len(result[0])
        return result[:, 0], result[:, 1]#返回的是当前位置的词，当前位置的词的上下文
